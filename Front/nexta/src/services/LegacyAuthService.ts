...
import { IsAuthRequest, IsAuthResponse } from "../features/auth/models/isAuth";
import { VerifyCodeRequest } from "../features/auth/models/verifyCode";
import { ChangePasswordRequest } from "../features/auth/models/changePassword";
import { ConfirmPhoneFormRequest, ConfirmPhoneFormResponse } from "../features/account/models/ConfirmPhoneForm";
...
export class AuthService{
    ...
    static async confirmPhone(data: ConfirmPhoneFormRequest): Promise<ApiResponse<ConfirmPhoneFormResponse, ErrorResponseModel>> {
        try{
            const response = await api.post<ConfirmPhoneFormResponse>(this.AUTH_ENPOINTS.CHANGEPHONE, data);
    ...
    static async isRegisterUser(email:string) : Promise<ApiResponse<IsRegisteredUserResponse, ErrorResponseModel>>{
        try{
            const response = await api.post<IsRegisteredUserResponse>(this.AUTH_ENPOINTS.ISREGISTERUSER, {email});
            return{
                success: true,
                data: response.data,
                status: response.status
